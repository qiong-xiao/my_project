...
class rr:
    ...
    def InitQue(self):
        ...
        if self.TxtPath != '':
            try:
                ...
                self.msg(self.window, f"发生错误：{str(e)}")

    def UpdateQue(self):
        # 清空表格
        self.window.main_ui.ready_tableWidget.clearContents()
        self.window.main_ui.in_tableWidget.clearContents()
        self.window.main_ui.out_tableWidget.clearContents()
        self.window.main_ui.wait_tableWidget.clearContents()

        if len(self.ReadyQue) != 0:
            for i in range(len(self.ReadyQue)):

                if self.ReadyQue[i][0] != self.window.main_ui.process_lineEdit.text():
        ...
        if len(self.WaitQue) != 0:
            if self.WaitQue[0][1][0] == 'W':
                if int(self.WaitQue[0][1][1:]) != 1:
                    ...
                    if len(self.WaitQue[0][1]) > 2:
                        if self.WaitQue[0][1][2] == 'O':
                            self.OutputQue.append(self.WaitQue[0])
                        elif self.WaitQue[0][1][2] == 'C':
    ...
    def MoveReadyToOtherQueues(self):
        if len(self.ReadyQue) != 0:

            for process in self.ReadyQue:

                if process[1][0] == 'I':
                    self.InputQue.append(process)
                    del self.ReadyQue[0]
                elif process[1][0] == 'O':
                    self.OutputQue.append(process)
                    del self.ReadyQue[0]
                elif process[1][0] == 'W':
                    self.WaitQue.append(process)
                    del self.ReadyQue[0]
